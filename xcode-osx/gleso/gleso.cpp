...
#define shader_source_vertex "#version 410\nuniform mat4 umvp;in vec4 apos;out vec4 coord;void main(){gl_Position=umvp*apos;coord=vec4((gl_Position.y+.5),0,0,1);}"
...
#define shader_source_fragment "#version 410\nin vec4 coord;out vec4 colr;void main(){colr=vec4(coord.x,coord.y,.1,1.);}"
