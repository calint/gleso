...
public:
    ...
    static const char*shader_type_name(const GLenum shader_type){
        if(shader_type==GL_VERTEX_SHADER)return"GL_VERTEX_SHADER";
        if(shader_type==GL_GEOMETRY_SHADER)return"GL_GEOMETRY_SHADER";
        if(shader_type==GL_FRAGMENT_SHADER)return"GL_FRAGMENT_SHADER";
...
//		LOGE("info log len: %d\n",infolen);
		...
		p("Could not compile shader %s:\n%s\n",shader_type_name(shader_type),buf);
		free(buf);
...
#define shader_source_vertex "#version 410\nuniform mat4 umvp;in vec4 apos;out vec4 coord;void main(){gl_Position=umvp*apos;coord=vec4(gl_Position.x,0,0,1);}"
...
#define shader_source_fragment "#version 410\nin vec4 coord;out vec4 colr;void main(){colr=vec4(coord.x+.5,coord.y+.5,.1,1.);}"
